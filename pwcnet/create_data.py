...
arguments_strOne = './images/1.jpg'
arguments_strTwo = './images/2.jpg'
arguments_strOut = './out4.flo'
...
def process_data(src_img_path, gt_img_path, outflow_path):

    tenOne = torch.FloatTensor(numpy.ascontiguousarray(
                    numpy.array(PIL.Image.open(src_img_path))[:, :, ::-1].transpose(2, 0, 1).astype(
                        numpy.float32) * (1.0 / 255.0)))
    
    tenTwo = torch.FloatTensor(numpy.ascontiguousarray(
                    numpy.array(PIL.Image.open(gt_img_path))[:, :, ::-1].transpose(2, 0, 1).astype(
                        numpy.float32) * (1.0 / 255.0)))
    
    tenOutput = estimate(tenTwo, tenOne)
    np_output = tenOutput.numpy().transpose(1,2,0)
    np_output = cv2.resize(np_output, (512,512))
    numpy.save(outflow_path, np_output)
