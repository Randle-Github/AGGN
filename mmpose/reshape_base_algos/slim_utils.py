...
def enlarge_box_tblr(roi_bbox, mask, ratio=0.4):
    ...
    if long_side == roi_width:
        new_left = left - int(long_side * (ratio / 2))
        new_left = 1 if new_left < 0 else new_left

        new_right = right + int(long_side * (ratio / 2))
        ...
        new_top = top - int(long_side * (ratio / 2))
        new_top = 1 if new_top < 0 else new_top

        new_bottom = bottom + int(roi_height * (ratio / 2))
        new_bottom = mask.shape[0]-2 if new_bottom > mask.shape[0] else new_bottom
...
def gen_PAF(joints, input_roi_box = None):

    # print(joints.shape[0])
    # assert joints.shape[0] == 18
    # assert joints.shape[1] == 3

    roi_box_width = input_roi_box[3] - input_roi_box[2]
    roi_box_height = input_roi_box[1] - input_roi_box[0]
    ...
    part_orders = [(5,11),(6,12),(5,7),(7,9),(6,8),(8,10),(11,13),(13,15),(12,14),(14,16)]

    map_list = []
    mask_list = []
    PAF_all = np.zeros(shape=(roi_box_height, roi_box_width, 2), dtype=np.float32)

    x_offset=input_roi_box[2]
    y_offset=input_roi_box[0]
    ...
    for c, pair in enumerate(part_orders):
        ...
        jointa[0] = jointa[0] - x_offset
        jointb[0] = jointb[0] - x_offset
        jointa[1] = jointa[1] - y_offset
        jointb[1] = jointb[1] - y_offset

        

        confidence_threshold = 0.5
        if jointa[2] > confidence_threshold and jointb[2] > confidence_threshold:
            canvas = np.zeros(shape =(roi_box_height, roi_box_width), dtype=np.uint8 )
            ...
            PAF = np.zeros(shape=(roi_box_height, roi_box_width, 2), dtype=np.float32)
            ...
            map_list.append(np.zeros(shape =(roi_box_height, roi_box_width), dtype=np.uint8))
            mask_list.append(np.zeros(shape =(roi_box_height, roi_box_width), dtype=np.uint8))

    PAF_all = cv2.GaussianBlur(PAF_all, (kernel_size*3, kernel_size*3), 0)
    map_list.append(PAF_all)

    map = np.dstack((map_list[0], map_list[1]))
    for i in range(len(map_list)):
        if i+2 < len(map_list):
            map = np.dstack((map, map_list[i + 2]))
    return PAF_all, map
...
def gen_skeleton_map(joints, stack_mode="column", input_roi_box = None):
    ...
    part_orders = [(5,6),(5,11),(6,12),(12,11),(5,7),(7,9),(6,8),(8,10),(11,13),(13,15),(12,14),(14,16)]
    ...
    for c, pair in enumerate(part_orders):
        if joints[pair[0]][2] < 0.5 or joints[pair[1]][2] < 0.5:
            continue
        tmp = map_cube[..., c].copy()
...
def joint_to_body_box(joints, thr):
    valid_joints = joints[joints[:,2] > thr]
    if np.shape(valid_joints)[0] <= 1:
        return None
    joint_left = int(np.min(valid_joints, axis=0)[0])
    joint_right = int(np.max(valid_joints, axis=0)[0])
    joint_top = int(np.min(valid_joints, axis=0)[1])
    joint_bottom = int(np.max(valid_joints, axis=0)[1])
